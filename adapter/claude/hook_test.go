...
func TestHookCommandPermissionRequestAwaitingInput(t *testing.T) {
	...
	cmd := exec.Command("sh", "-c", hc.Command)
	cmd.Env = append(os.Environ(), "AGENTRUNTIME_SESSION_ID=correlate-claude-2", "AGENTRUNTIME_HOOK_ENDPOINT="+server.URL)
	cmd.Stdin = strings.NewReader(hookStdin)
...
func TestHookCommandStopFailureMapsToError(t *testing.T) {
	...
	cmd := exec.Command("sh", "-c", hc.Command)
	cmd.Env = append(os.Environ(), "AGENTRUNTIME_SESSION_ID=correlate-claude-3", "AGENTRUNTIME_HOOK_ENDPOINT="+server.URL)
	cmd.Stdin = strings.NewReader(hookStdin)
