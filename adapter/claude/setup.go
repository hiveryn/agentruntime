...
// generated hook command (see hook.go). Claude owns and rewrites settings.json
// and drops unknown fields like markerField, so detection must key on the
// command string the agent preserves, not on a custom field it may strip.
