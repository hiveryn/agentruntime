...
// Codex hook events. The generated command is a self-contained node script that
...
// $AGENTRUNTIME_HOOK_ENDPOINT/codex. When that env var is empty (a session not
// launched with StartRequest.HookEndpoint) it posts nothing. Only node is
// required at runtime — both Codex and Claude CLI are Node.js applications.
