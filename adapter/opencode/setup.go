...
// Events POST to $AGENTRUNTIME_HOOK_ENDPOINT/opencode, resolved per session so
// concurrent callers never redirect each other's sessions. Sessions launched
// without a hook endpoint post nothing.
const endpoint = process.env?.AGENTRUNTIME_HOOK_ENDPOINT ?? ""
...
func renderPlugin(marker string) (string, error) {
