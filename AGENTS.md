...
- `adapter/codex`: uses `AGENTRUNTIME_SESSION_ID` for caller-owned correlation; hook setup writes `hooks.json` in the Codex config dir (`ConfigRoot` reads `CODEX_HOME`, default `~/.codex`); `codex.HookCommand(endpoint)` generates a self-contained node hook command. Resume: `codex resume` (bare = interactive picker) or `codex resume <id>` (specific); bare resume suppresses `Prompt` because `codex resume` treats the next positional as `SESSION_ID`.
